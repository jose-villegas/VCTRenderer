...
void ImGui_ImplGlfwGL3_NewFrame()
{
    ...
    io.DeltaTime = g_Time > 0.0 ? (float)(current_time - g_Time) : (float)(
                       1.0f / 60.0f);
    g_Time = current_time;
    ...
    if(glfwGetWindowAttrib(g_Window, GLFW_FOCUSED))
    {
        ...
        mouse_x *= (float)display_w /
                   w;                        // Convert mouse coordinates to pixels
    ...
    for(int i = 0; i < 3; i++)
    {
        io.MouseDown[i] = g_MousePressed[i] ||
                          glfwGetMouseButton(g_Window,
                                             i) != 0;    // If a mouse press event came, always pass it as "mouse held this frame", so we don't miss click-release events that are shorter than 1 frame.
