/**
 *  @example oglplus/014_multi_cube_ssbo.cpp
 *  @brief Shows how to transform and draw multiple instances of a cube
 *
 *  @oglplus_screenshot{014_multi_cube_ssbo}
 *
...
public:
	 ...
	 , camera_matrix(prog)
	{
		...
		vs.Source(
			...
			"layout (std430) buffer ModelBlock {"
			"	mat4 ModelMatrices[];"
			"};"
			"in vec4 Position;"
			"out vec3 vertColor;"
		...
		// make the matrices
		{
			...
			ShaderStorageBlock model_block(prog, "ModelBlock");
			model_block.Binding(0);

			block_buf.Bind(Buffer::Target::ShaderStorage);
			Buffer::Data(
