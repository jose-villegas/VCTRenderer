...
//
// NOTE BY OGLPLUS AUTHORS
//   Removed parts of the documentation, the examples and C++-ified some
//   parts below, removed some warnings, converted line endings.
//   See http://nothings.org/ for the original header file.
...
#pragma clang diagnostic ignored "-Wold-style-cast"
   ...
   std::qsort(data,num_items,item_size,compare_func)
   #endif
   ...
   #ifndef STBTT_malloc
   #include <cstdlib>
   #define STBTT_malloc(x,u)  std::malloc(x)
   #define STBTT_free(x,u)    std::free(x)
   #endif
...
int stbtt_FindGlyphIndex(const stbtt_fontinfo *info, int unicode_codepoint)
{
   ...
   if (format == 0) { // apple byte encoding
      ...
      while (entrySelector) {
         searchRange >>= 1;
         if (unicode_codepoint > ttUSHORT(data + search + searchRange*2))
            ...
            if (matchlen >= 0) {
               ...
               if (i+1 < count && ttUSHORT(fc+loc+12+6) == next_id && ttUSHORT(fc+loc+12) == platform && ttUSHORT(fc+loc+12+2) == encoding && ttUSHORT(fc+loc+12+4) == language) {
                  ...
                  if (slen2 == 0) {
                     ...
                     ++matchlen;
                     if (stbtt_CompareUTF8toUTF16_bigendian((char*) (name+matchlen), nlen-matchlen, (char*)(fc+stringOffset+off2),slen2))
                        return 1;
                  }
               } else {
